// <copyright file="BoardFancy.cs" company="JoAnn D. Peeler">
...
namespace Pedantic.Chess
{
    ...
    public sealed partial class Board
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Bitboard GetRookAttacksFancy(SquareIndex square, Bitboard blockers)
        {
            return fancyLookupTable[FancyRookIndex(square, blockers)];
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Bitboard GetBishopAttacksFancy(SquareIndex square, Bitboard blockers)
        {
            return fancyLookupTable[FancyBishopIndex(square, blockers)];
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Bitboard GetQueenAttacksFancy(SquareIndex square, Bitboard blockers)
        {
            return fancyLookupTable[FancyRookIndex(square, blockers)] |
                   fancyLookupTable[FancyBishopIndex(square, blockers)];
        }

        private static int FancyRookIndex(SquareIndex square, Bitboard blockers)
        {
            FancyHash m = fancyRookMagics[(int)square];
            return m.Offset + (int)(((blockers | m.Mask) * m.Hash) >> (MAX_SQUARES - 12));
        }

        private static int FancyBishopIndex(SquareIndex square, Bitboard blockers)
        {
            FancyHash m = fancyBishopMagics[(int)square];
            return m.Offset + (int)(((blockers | m.Mask) * m.Hash) >> (MAX_SQUARES - 9));
        }

        private static Bitboard RelevantBishopSee(File x, Rank y)
        {
            // result attacks bitboard
            Bitboard attacks = (Bitboard)0ul;
            ...
            for (r = y + 1, f = x + 1; r <= Rank.Rank7 && f <= File.FileG; r++, f++) attacks |= new Bitboard(ChessMath.ToSquareIndex(f, r));
            for (r = y - 1, f = x + 1; r >= Rank.Rank2 && f <= File.FileG; r--, f++) attacks |= new Bitboard(ChessMath.ToSquareIndex(f, r));
            for (r = y + 1, f = x - 1; r <= Rank.Rank7 && f >= File.FileB; r++, f--) attacks |= new Bitboard(ChessMath.ToSquareIndex(f, r));
            for (r = y - 1, f = x - 1; r >= Rank.Rank2 && f >= File.FileB; r--, f--) attacks |= new Bitboard(ChessMath.ToSquareIndex(f, r));

            // return attack map
            return attacks;
        }

        private static Bitboard RelevantRookSee(File x, Rank y)
        {
            // result attacks bitboard
            Bitboard attacks = (Bitboard)0ul;
            ...
            for (r = y + 1; r <= Rank.Rank7; r++) 
            {
                attacks |= new Bitboard(ChessMath.ToSquareIndex(x, r));
            }

            for (r = y - 1; r >= Rank.Rank2; r--)
            {
                attacks |= new Bitboard(ChessMath.ToSquareIndex(x, r));
            }

            for (f = x + 1; f <= File.FileG; f++)
            {
                attacks |= new Bitboard(ChessMath.ToSquareIndex(f, y));
            }

            for (f = x - 1; f >= File.FileB; f--)
            {
                attacks |= new Bitboard(ChessMath.ToSquareIndex(f, y));
            }
        ...
        {
            for (File x = File.FileA; x <= File.FileH; x++)
            {
                for (Rank y = Rank.Rank1; y <= Rank.Rank8; y++)
                {
                    SquareIndex sq = ChessMath.ToSquareIndex(x, y);
                    Bitboard rookMask = RelevantRookSee(x, y);
                    Bitboard bishopMask = RelevantBishopSee(x, y);

                    int cnt = rookMask.PopCount;
                    for (ulong i = 0; i < (1ul << cnt); i++)
                    {
                        Bitboard blockers = (Bitboard)BitOps.ParallelBitDeposit(i, rookMask);
                        fancyLookupTable[FancyRookIndex(sq, blockers)] = GetRookAttacks(sq, blockers);
                    }

                    cnt = bishopMask.PopCount;
                    for (ulong i = 0; i < (1ul << cnt); i++)
                    {
                        Bitboard blockers = (Bitboard)BitOps.ParallelBitDeposit(i, bishopMask);
        ...
        static readonly FixedArray<Bitboard> fancyLookupTable = new (88507, true);

        private static readonly FixedArray<FancyHash> fancyBishopMagics = new (MAX_SQUARES)
        {
        ...
        private static readonly FixedArray<FancyHash> fancyRookMagics = new (MAX_SQUARES)
        {
