// <copyright file="BoardPext.cs" company="JoAnn D. Peeler">
...
namespace Pedantic.Chess
{
    ...
    public sealed partial class Board
    {
        ...
        {
            public Bitboard* pAtkRook;
            public Bitboard* pAtkBish;
            public Bitboard MaskRook;
            public Bitboard MaskBish;

            public PextEntry(int offsetRook, int offsetBish, Bitboard maskRook, Bitboard maskBish)
            {
                fixed (Bitboard* ptr = &attacks[offsetRook])
                {
                    pAtkRook = ptr;
                }

                fixed (Bitboard* ptr = &attacks[offsetBish])
                {
                    pAtkBish = ptr;
                }
                MaskRook = maskRook;
        ...
        public static unsafe Bitboard GetBishopAttacksPext(SquareIndex square, Bitboard blockers)
        {
            ref PextEntry entry = ref entries[(int)square];
            return entry.pAtkBish[BitOps.ParallelBitExtract(blockers, entry.MaskBish)];
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe Bitboard GetRookAttacksPext(SquareIndex square, Bitboard blockers)
        {
            ref PextEntry entry = ref entries[(int)square];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe Bitboard GetQueenAttacksPext(SquareIndex square, Bitboard blockers)
        {
            ref PextEntry entry = ref entries[(int)square];
            return entry.pAtkBish[BitOps.ParallelBitExtract(blockers, entry.MaskBish)] |
        ...
        private static PextEntry CreateEntry(SquareIndex sq, Bitboard maskRook, Bitboard maskBish)
        {
            int offsetRook = attacks.Count;
            int cnt = maskRook.PopCount;
            for (ulong i = 0; i < (1ul << cnt); i++)
            {
                Bitboard blockers = (Bitboard)BitOps.ParallelBitDeposit(i, maskRook);
                attacks.Add(GetRookAttacks(sq, blockers));
            }

            int offsetBish = attacks.Count;
            cnt = maskBish.PopCount;
            for (ulong i = 0; i < (1ul << cnt); i++)
            {
                Bitboard blockers = (Bitboard)BitOps.ParallelBitDeposit(i, maskBish);
        ...
        {
            for (SquareIndex sq = SquareIndex.A1; sq <= SquareIndex.H8; sq++)
            {
                var coords = sq.ToCoords();
                entries[(int)sq] = 
                    CreateEntry(sq, RelevantRookSee(coords.File, coords.Rank), RelevantBishopSee(coords.File, coords.Rank));
            }
        }

        public static readonly bool IsPextSupported;
        private static readonly FixedArray<Bitboard> attacks = new(107648);
        private static readonly FixedArray<PextEntry> entries = new(MAX_SQUARES, true);
