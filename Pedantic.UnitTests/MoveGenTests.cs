namespace Pedantic.UnitTests
{
    ...
    public class MoveGenTests
    {
        ...
        public void NormalMoveGenTest(string fen)
        {
            MoveList list = new();
            SearchStack ss = new();
            History hist = new(ss);
            List<GenMove> list1 = new List<GenMove>();
            List<GenMove> list2 = new List<GenMove>();

            Board board = new Board(fen);
            foreach (GenMove mv in board.Moves(0, hist, ss, list, Move.NullMove))
            {
                list1.Add(mv);
            }

            MoveGen gen = new(MoveGenType.Normal, board, 0, hist, ss, list, Move.NullMove);
            foreach (GenMove mv in gen)
            {
                list2.Add(mv);
            }
            ...
            {
                Assert.AreEqual(list1[n], list2[n]);
            }
        }
		...
		[DataRow("3br1k1/p1pn3p/1p3n2/5pNq/2P1p3/1PN3PP/P2Q1PB1/4R1K1 w - - 0 23")]
		[DataRow("2r2b2/5p2/5k2/p1r1pP2/P2pB3/1P3P2/K1P3R1/7R w - - 23 93")]
        public void QSearchMoveGenTest(string fen)
        {
            MoveList list = new();
            SearchStack ss = new();
            History hist = new(ss);
            ...
            Board board = new Board(fen);
            foreach (GenMove mv in board.QMoves(0, 0, hist, ss, list, Move.NullMove))
            {
                list1.Add(mv);
            }

            MoveGen gen = new(MoveGenType.QSearch, board, 0, hist, ss, list, Move.NullMove, 0);
            foreach (GenMove mv in gen)
            {
                list2.Add(mv);
            }
        ...
        [TestMethod]
        [DataRow("1rb1rn1k/p3q1bp/2p3N1/2p1p3/2P1P3/PP1RQNP1/1B3P2/4R1K1 b - - 4 23")]
        public void EvasionMoveGenTest(string fen)
        {
            MoveList list = new();
            SearchStack ss = new();
            History hist = new(ss);
            ...
            Board board = new Board(fen);
            foreach (GenMove mv in board.EvasionMoves(0, hist, ss, list, Move.NullMove))
            {
                list1.Add(mv);
            }

            MoveGen gen = new(MoveGenType.Evasion, board, 0, hist, ss, list, Move.NullMove);
            foreach (GenMove mv in gen)
            {
                list2.Add(mv);
            }
