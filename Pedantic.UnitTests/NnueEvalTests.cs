namespace Pedantic.UnitTests
{
    ...
    public class NnueEvalTests
    {
        ...
        public void ComputeTests(string fen)
        {
            ...
            short score2 = nnueEval.ComputeUncached(bd);
        ...
        public void AddPieceTest()
        {
            ...
            bd.MakeMove(mv);
            short score2 = nnue.Compute(bd);
            Assert.AreNotEqual(score2, score);

            bd.UnmakeMove();
            score2 = nnue.Compute(bd);
        ...
        public void AddRemPieceTest()
        {
            ...
            score[ply++] = nnue.Compute(bd);

            while (ply < 8)
            {
                moveList.Clear();
                bd.GenerateMoves(moveList);
                int n = Random.Shared.Next(0, moveList.Count);
                while (!bd.MakeMove(moveList[n]))
                {
                    moveList.Remove(moveList[n]);
                    n = Random.Shared.Next(0, moveList.Count);
                }
                score[ply++] = nnue.Compute(bd);
            }
            ...
            {
                short score2 = nnue.Compute(bd);
                Assert.AreEqual(score[ply], score2);
                bd.UnmakeMove();
            }
        }
    }
